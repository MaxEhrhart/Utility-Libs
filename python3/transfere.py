...
def recebe_argumentos():
    ...
    parser.add_argument("--pasta_destino", "--pd", help=u"Define a pasta destino para os diretórios a serem baixados. Passar caminho completo.", required=False,type=str)
    params = parser.add_mutually_exclusive_group(required=False)
    params.add_argument("--recente", "--r", help=u"Seleciona a execução mais recente automaticamente.", required=False,action="store_true")
    params.add_argument("--todos", "--t", help=u"Baixa todas as pastas de execuções se existir para a pasta onde o script está localizado atualmente.", required=False,action="store_true")
...
if __name__ == "__main__":
    ...
    pasta_execucoes = list(set([arquivo.rsplit("/",2)[0][t_dir_extratos+1:] for arquivo in arquivos if len(arquivo.rsplit("/",2)[0][t_dir_extratos+1:]) == 12]))
    ...
    with ssh.open_sftp() as sftp:
        ...
        for arquivo in arquivos:
            ...
            arquivo_destino = pasta_destino + arquivo.rsplit("/",1)[0][t_dir_extratos:] + "/" + arquivo.rsplit("/",1)[1]
