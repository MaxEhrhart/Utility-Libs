...
def cria_grafico(meses, valores, filename):
    ...
    plt.box(on=None)
    plt.tick_params(axis='x', labeltop=False, left=False, top=False, right=False, bottom=False, labelleft=False,labelright=False, labelbottom=False)
    plt.tick_params(axis='y', labeltop=False, left=False, top=False, right=False, bottom=False, labelleft=False,labelright=False, labelbottom=False)
...
def trata_dados(tipo_execucao, ano_mes, selecao_recente=False):
    ...
    df_ex["cd_tp_exec"] = df_ex[["cd_execucao","dt_carga"]].apply(lambda x: " - ".join(["{0:03d}".format(x[0]),x[1]]), axis="columns")
    ...
    codigos_exec = pd.unique(df_ex[(df_ex["tp_execucao"] == tipo_execucao.lower().replace(u"é", "e")) & (df_ex["ano_mes_venda"] == ano_mes)]["cd_execucao"])
    ...
    if not selecao_recente:
        ...
        while (sel_exec not in codigos_exec) or (len(sel_exec) < 1):
            console_print(u"Selecione entre os seguintes codigos de execução:\n" + df_ex["cd_tp_exec"].sort_values().to_string(index=False) + "\n")
    ...
    console_print(u"Preenchendo campos nulos do dataframe com registros brutos pela string \"null\" para não perder registros na transformação em pivot ...")
    ...
    pivot_df = pd.pivot_table(df,
        index=colunas_chave + ["nm_consultor", "nu_matricula_rh", "nm_cargo", "nm_unidade", "nm_gerente", "email_gestor"],
        columns=["nm_familia"],
        values=["vl_venda", "vl_calc_comissao_final", "vl_atingimento_taxa", "vl_atingimento_prazo","vl_calc_ri", "total_vidas"],
        aggfunc=np.sum
    )
    ...
    df["comissao"] = df["comissao"].apply(lambda x: x if x <= LIMITE_COMISSAO else LIMITE_COMISSAO)  # PENSAR NUMA VERSAO HISTORICO FUTURAMENTE
    ...
    for mes in meses_ano_comercial[:meses_ano_comercial.index(ano_mes)]:
        ...
        tp_execucao = "ajuste" if \
            not pd.isnull(df_historico_mes[df_historico_mes["tp_execucao"] == "ajuste"]["cd_execucao"].drop_duplicates().max()) \
            else "oficial" if not pd.isnull(df_historico_mes[df_historico_mes["tp_execucao"] == "oficial"]["cd_execucao"].drop_duplicates().max()) \
            else "vazio"
        ...
        if tp_execucao != "vazio":
            ...
            codigo_execucao = df_historico_mes[df_historico_mes["tp_execucao"] == tp_execucao]["cd_execucao"].drop_duplicates().max()
    ...
    df_mes["historico"] = df_mes[["historico","comissao"]].apply(lambda l: l["historico"].lstrip(";") + ";{0:.2f}".format(l["comissao"]),axis=1)
    ...
    # Tratamentos
    campos_flag = ["flg_prazo_beneficio", "flg_taxa_beneficio", "flg_prazo_gd", "flg_taxa_gd", "flg_prazo_ir", "flg_taxa_ir"]
    ...
    for campo in campos_moeda:
        df_mes[campo] = df_mes[campo].astype(str).replace("nan","-").apply(lambda x: "-" if x in ("-","0.0") else valor_real(float(x)))

    # Trata campos porcentagem para preencher coluna vazia com "-".
    for campo in campos_flag:
        df_mes[campo] = df_mes[campo].astype(str).apply(lambda x: "{0}%".format(100 * (float(x) - 1)) if x not in("nan","0.0") else "-")
...
def cria_extrato(tipo_execucao,ano_mes,consultor):
    ...
    def valor_venda(c, consultor, font="Times-Roman", fontsize=12):
        ...
        # TEMPORARIO
        c.drawCentredString( 685, 308, unicode(formata_valor_venda(valor_real(100*real_valor(consultor[u"comissao_ri"])))) if  consultor[u"comissao_ri"] != "-" else "-")
        return c
...
def recebe_argumentos():
    ...
    # Selecao dos dados
    parser.add_argument("--recente", "--r", help=u"Seleciona a execucao mais recente automaticamente.", required=False,action=u"store_true")
    # Lê argumentos
...
if __name__ == "__main__":
    ...
    for i, consultor in enumerate(consultores_vendedores):
        ...
        consultor[u"grafico"] = cria_grafico(meses_ano_comer cial, valores, filename="{0}.png".format(i))
        # Cria PDF
            ...
            with codecs.open(CSV_OUTPUT_PATH + "/" + csv_filename, "w", "utf-8") as f:
                for consultor in consultores_vendedores:
                    ...
                    f.write(";".join(
                        ...
                        , "0025"
                        , "0,00" if consultor["comissao"] == "-" else consultor["comissao"].lstrip("R$").strip(" ").replace(".", "")])
                        + "\n"
                    )
                f.flush()
                f.close()
